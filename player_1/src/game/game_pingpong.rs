...
pub struct MainState {
    ...
    player_1_score: f32,
    player_2_score: f32,
    rx: common_lib::Receiver<GameData>,
}

impl MainState {
    pub fn new(ctx: &mut Context, rx: common_lib::Receiver<GameData>) -> Self {
...
impl event::EventHandler for MainState {
    fn update(&mut self, ctx: &mut Context) -> GameResult {
        ...
        if let Ok(data) = self.rx.try_recv() {
            match data {
                GameData::Data((ball_x, ball_y), pos_x, pos_y, score_x, score_y) => {
                    self.ball_pos.x = ball_x;
                    self.ball_pos.y = ball_y;
                    self.player_1_pos.y = pos_x;
                    self.player_1_score = score_x;
                    self.player_2_pos.y = pos_y;
                    self.player_2_score = score_y;
                }
            }
        }
...
pub fn game_pingpong_run(rx: common_lib::Receiver<GameData>) {
