...
use library::common_lib;
use library::common_lib::GameData;
use library::common_lib::{Receiver, Sender, UserCommand};
...
impl MainState {
    ...
    pub fn get_game_data(&self) -> Result<GameData, io::Error> {
        Ok(GameData::Data(
...
impl event::EventHandler for MainState {
    fn update(&mut self, ctx: &mut Context) -> GameResult {
        ...
        if let Ok(game_data) = self.get_game_data() {
            if let Err(e) = self.tx.try_send(game_data) {
                println!("Try send Error = {:?}", e);
            }
        }
...
pub fn game_pingpong_run(rx: Receiver<UserCommand>, tx: library::common_lib::Sender<GameData>) {
...
pub async fn pingpong_get_cmd(tx_cmd: Sender<UserCommand>, cmd: UserCommand) -> io::Result<()> {
    if let Err(e) = tx_cmd.send(cmd).await {
        println!("Error {:?}", e);
    }
    Ok(())
}
